...
class CouponShapeBorder extends ShapeBorder {
  ...
  CouponShapeBorder(
      {this.holeCount = 6, this.lineRate = 0.7, this.dash = true, this.color = Colors.white});
  ...
  void _formHoleBottom(Path path, Rect rect) {
    path.addArc(
        Rect.fromCenter(center: Offset(lineRate * rect.width, rect.height), width: 13.0, height: 13.0),
        pi,
        pi);
  }
  ...
  void paint(Canvas canvas, Rect rect, {TextDirection? textDirection}) {
    ...
    if (dash) {
      ...
      //实线
      canvas.drawLine(
          Offset(lineRate * rect.width, d / 2), Offset(lineRate * rect.width, rect.height - d / 2), paint);
    }
  }
